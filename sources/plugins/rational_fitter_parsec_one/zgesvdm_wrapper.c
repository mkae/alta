...
dague_object_t* dplasma_zgesvd_multiple_New( gesvdm_args_t *args )
{
    dague_zgesvd_multiple_object_t* object;
     ...
     */
    object = dague_zgesvd_multiple_new( args, (dague_ddesc_t*)(A),
                                        (dague_ddesc_t*)(args->ci),
                                        *(args->qrtree), args->ib,
                                        NULL, NULL/*, NULL*/ );

    object->p_work = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t));
    dague_private_memory_init( object->p_work, args->ib * A->nb * sizeof(dague_complex64_t) );

    object->p_tau = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t));
    dague_private_memory_init( object->p_tau, A->nb * sizeof(dague_complex64_t) );

    /* object->p_superb = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t)); */
    /* dague_private_memory_init( object->p_superb, 5 * dague_imin(A->nb, A->m) * sizeof(dague_complex64_t) ); */

    /* Default type */
    dplasma_add2arena_tile( object->arenas[DAGUE_zgesvd_multiple_DEFAULT_ARENA],
                            A->mb*A->nb*sizeof(dague_complex64_t),
                            DAGUE_ARENA_ALIGNMENT_SSE,
                            MPI_DOUBLE_COMPLEX, A->mb );

    /* Upper triangular part of tile with diagonal */
    dplasma_add2arena_upper( object->arenas[DAGUE_zgesvd_multiple_UPPER_TILE_ARENA],
                             A->mb * A->nb * sizeof(dague_complex64_t),
                             DAGUE_ARENA_ALIGNMENT_SSE,
                             MPI_DOUBLE_COMPLEX, A->mb, 1 );

    /* /\* Lower triangular part of tile without diagonal *\/ */
    /* dplasma_add2arena_lower( object->arenas[DAGUE_zgesvd_multiple_LOWER_TILE_ARENA], */
    /*                          A->mb*A->nb*sizeof(dague_complex64_t), */
    /*                          DAGUE_ARENA_ALIGNMENT_SSE, */
    /*                          MPI_DOUBLE_COMPLEX, A->mb, 0 ); */

    /* Vector */
    dplasma_add2arena_rectangle( object->arenas[DAGUE_zgesvd_multiple_VECTOR_ARENA],
...
int dplasma_zgesvd_multiple( dague_context_t *dague,
                             gesvdm_args_t *args )
{
    dague_object_t *dague_zgesvd_multiple = NULL;

    dague_zgesvd_multiple = dplasma_zgesvd_multiple_New( args );
    dague_enqueue(dague, (dague_object_t*)dague_zgesvd_multiple);
...
void
dplasma_zgesvd_multiple_Destruct( dague_object_t *o )
{
    dague_zgesvd_multiple_object_t *dague_zgesvd_multiple = (dague_zgesvd_multiple_object_t *)o;
    ...
    DAGUE_INTERNAL_OBJECT_DESTRUCT(dague_zgesvd_multiple);
}
